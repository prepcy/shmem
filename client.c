...
int main()
{
	...
	long long num_t = 10000000;

	while(1) {
		if (num_t == total_count)
			while(1);
		/* 在队列中等待、获取一个空闲的帧 */
		xdma_frame = wait_free_xdma_frame(xdma_queue);

		*xdma_frame->len = IP_SIZE;
		memset(xdma_frame->data, 0xaa, *xdma_frame->len);
		xdma_frame->data[0] = total_count & 0Xff;
		total_len += *xdma_frame->len;
		total_count ++;

		/* 通知处理线程有新的一帧准备好 */
		xdma_used_frame_inc(xdma_queue);
		xdma_frame = NULL;
    }
}
